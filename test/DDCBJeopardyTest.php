 ...
 **/
abstract class DDCBJeopardyTest extends \PHPUnit_Extensions_Database_TestCase {
	/**
...
// THESE TABLES *MUST* BE LISTED IN THE SAME ORDER THEY WERE CREATED!!!!
		...
		$dataset->addTable("alert");
		$dataset->addTable("student");
		$dataset->addTable("comment");
		$dataset->addTable("attendance");
		$dataset->addTable("game");
		$dataset->addTable("player");
		$dataset->addTable("category");
		$dataset->addTable("qna");
		$dataset->addTable("badCategoryName");
		$dataset->addTable("gameQna");
		$dataset->addTable("score");
		return($dataset);
	}
...
// connect to mySQL and provide the interface to PHPUnit
			$config = readConfig("/etc/apache2/capstone-mysql/jeopardy.ini");
